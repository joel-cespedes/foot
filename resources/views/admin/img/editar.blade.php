    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                ...
                {!! Form::model($pro,['url'=>['adm/img',$pro->id], 'method'=>'patch', 'files' => true , 'class'=>'form-horizontal formulario']) !!}
                @include('admin.img.form')
                {!! Form::close() !!}
            </div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        $(add_button1).click(function(e){ //on add input button click
            ...
            if(x < max_fields){ //max input box allowed
                ...
                $(wrapper1).append(
                    ...
                    '<input type="hidden" name="nuevo" value="3" />'+
                    '</div>'+
                    '</div>'+
        ...
        $( ".target" ).change(function() {
            window.location.href = "http://localhost:8000/adm/img/"+$(this).val()+"/edit";
        });
        var url =     window.location.href
        var division =  url.split("/");
        var seleccionado= $('.target option[value='+division[5]+']');
